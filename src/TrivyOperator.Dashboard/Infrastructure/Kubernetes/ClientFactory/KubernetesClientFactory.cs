...
public class KubernetesClientFactory : IKubernetesClientFactory
{
    ...
    {
        this.logger = logger;
        clients = new Dictionary<ContextName, k8s.Kubernetes>();
    ...
    public k8s.Kubernetes GetClient(ContextName context)
    {
        if (context.IsUnset)
        {
        ...
        if (clients.TryGetValue(context, out k8s.Kubernetes? client))
        {
            return client;
        }
    ...
    private void InitializeExplicitContexts(KubernetesOptions options)
    {
        ...
        else
        {
            if (KubernetesClientConfiguration.IsInCluster())
            {
                ...
                clients[defaultContextName] = new k8s.Kubernetes(defaultConfig);
            }
            else
            {
    ...
    private void LoadSingleDefaultContextFromFile(string kubeConfigFileName)
    {
        ...
        clients.Clear();
        clients[contextName] = new k8s.Kubernetes(config);
    ...
    private void SetContextsFromConfig(string? kubeConfigFileName = null)
    {
        ...
        foreach (Context? ctx in loadedKubeConfig.Contexts)
        {
            try
            {
                ...
                config.AddJsonOptions(JsonUtils.ConfigureJsonSerializerOptions);
                clients[new ContextName(ctx.Name)] = new k8s.Kubernetes(config);
            }
            catch (Exception ex)
            {
