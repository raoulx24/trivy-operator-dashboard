    ...
    where TValue : IKubernetesObject<V1ObjectMeta>, IMetadata<V1ObjectMeta>
{
    ...
    public ConcurrentDictionary<string, TValue> this[string key]
    {
        get
        {
            IList<TValue> items = FetchByKeyAsync(key).ConfigureAwait(false).GetAwaiter().GetResult();
    ...
    // -----------------------
    public IEnumerable<string> Keys => BuildOuter(FetchAllAsync().ConfigureAwait(false).GetAwaiter().GetResult()).Keys;
    ...
    public int Count => BuildOuter(FetchAllAsync().ConfigureAwait(false).GetAwaiter().GetResult()).Count;
    ...
    public bool TryUpdate(
        string key,
        ConcurrentDictionary<string, TValue> newValue,
        ConcurrentDictionary<string, TValue> comparisonValue
    ) => false;
    ...
    protected static IDictionary<string, ConcurrentDictionary<string, TValue>> BuildOuter(IList<TValue> items) => items
        .GroupBy(item => CacheUtils.GetCacheRefreshKey(item))
        .ToDictionary(g => g.Key, g => BuildInner([.. g,]));
}
