...
public class InfraAssessmentReportService(IConcurrentDictionaryCache<InfraAssessmentReportCr> cache) : IInfraAssessmentReportService
{
        ...
        IEnumerable<int>? excludedSeverities = null)
    {
        ...
        IEnumerable<InfraAssessmentReportCr> cachedValues = [.. cache
            .Where(kvp => string.IsNullOrEmpty(namespaceName) || kvp.Key == namespaceName)
            .SelectMany(kvp => kvp.Value.Values),];

        IEnumerable<InfraAssessmentReportDto> values = cachedValues
            .Select(cr => cr.ToInfraAssessmentReportDto())
            .Select(
                dto =>
                {
    ...
    public Task<InfraAssessmentReportDto?> GetInfraAssessmentReportDtoByUid(Guid uid)
    {
        ...
        {
            if (cache.TryGetValue(namespaceName, out ConcurrentDictionary<string, InfraAssessmentReportCr>? InfraAssessmentReportCrs))
            {
                if (InfraAssessmentReportCrs.TryGetValue(uid.ToString(), out InfraAssessmentReportCr? InfraAssessmentReportCr))
                {
                    return Task.FromResult<InfraAssessmentReportDto?>(InfraAssessmentReportCr.ToInfraAssessmentReportDto());
                }
            }
        }
        ...
        string? namespaceName = null)
    {
        IEnumerable<InfraAssessmentReportCr> cachedValues = [.. cache
            .Where(kvp => string.IsNullOrEmpty(namespaceName) || kvp.Key == namespaceName)
            .SelectMany(kvp => kvp.Value.Values),];
        IEnumerable<InfraAssessmentReportDenormalizedDto> values = cachedValues
            .SelectMany(car => car.ToInfraAssessmentReportDetailDenormalizedDtos());
